...
interface SettingsContextType {
  ...
  activeTab: 'TIME' | 'THEME' | 'EDOM'
  setActiveTab: (tab: 'TIME' | 'THEME' | 'EDOM') => void
}
...
export const SettingsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [activeTab, setActiveTab] = useState<'TIME' | 'THEME' | 'EDOM'>('EDOM')
  ...
  useEffect(() => {
    ...
    const savedTab = localStorage.getItem('settings_active_tab') as 'TIME' | 'THEME' | 'EDOM'
    if (savedTab) {
  ...
  // Update Active Tab
  const handleSetActiveTab = (tab: 'TIME' | 'THEME' | 'EDOM') => {
    setActiveTab(tab)
